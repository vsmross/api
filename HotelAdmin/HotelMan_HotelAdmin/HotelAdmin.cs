...
// Changed or add/remove something here for dummy commit to trigger a pipeline
...
public class HotelAdmin
{
    ...
    public async Task<APIGatewayProxyResponse> AddHotel(APIGatewayProxyRequest request, ILambdaContext context)
    {
        ...
        try
        {
            ...
            // send new hotel creation SNS notification
            if (!string.IsNullOrEmpty(Environment.GetEnvironmentVariable("snsTopicArn")))
            {
                ...
                Console.WriteLine($"Connecting to SNS topic : '{topicARN}'");
                ...
                Console.WriteLine($"Done sending notification SNS topic : '{topicARN}'");
            }
            else
            {
                Console.WriteLine($"SNS topic Not set in Environment variable");
            }
        }
        catch (Exception e)
        {
